...
const Step2 = () => {
  ...
  useEffect(() => {
    const savedAssets = JSON.parse(localStorage.getItem('contentAssets') || '[]');
    const savedFunnelStages = JSON.parse(localStorage.getItem('funnelStages') || JSON.stringify(funnelStages));
  ...
  useEffect(() => {
    localStorage.setItem('contentAssets', JSON.stringify(contentAssets));
  }, [contentAssets]);

  useEffect(() => {
    localStorage.setItem('funnelStages', JSON.stringify(funnelStages));
  }, [funnelStages]);
